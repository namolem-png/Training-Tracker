...
export class StatisticsView {
    ...
    async renderStreakPlaque(model, statsService) {
        ...
        const n = await (statsService || this.statsService).getConsecutiveWeeks(); // getConsecutiveWeeks теперь сам загружает все тренировки
    ...
    async populateExerciseSelects(model, statsService) {
        const grouped = await (statsService || this.statsService).getGroupedExerciseLibrary(); // getGroupedExerciseLibrary теперь сам загружает все тренировки
    ...
    async updateProgressChart(model, statsService) {
        ...
        const s = statsService || this.statsService; // Используем переданный сервис или собственный
        const data = await s.getExerciseProgressData(exerciseId, this.currentProgressMetric, this.currentProgressInterval); // Больше не передаем cachedAllWorkouts
    ...
    async updateVolumeChart(model, statsService) {
        ...
        if (this.currentVolumeMetric === 'total_tonnage') {
            this.volumeChart.render(await s.getOverallTonnageData(this.currentVolumeInterval), 'bar'); // Больше не передаем cachedAllWorkouts
        } else if (this.currentVolumeMetric === 'cat_tonnage') {
            const rawData = await s.getCategoryTonnageData(this.currentVolumeInterval); // Больше не передаем cachedAllWorkouts
            ...
            this.volumeChart.render(await s.getExerciseProgressData(exerciseId, 'tonnage', this.currentVolumeInterval), 'bar'); // Больше не передаем cachedAllWorkouts
        }
    }
    ...
    async updateBalanceChart(model, statsService) {
        ...
        const data = await (statsService || this.statsService).getCategorySetsData(this.currentBalanceInterval); // Больше не передаем cachedAllWorkouts
        this.balanceChart.render(data);
    }

    async updateWeeklyWidget(model, statsService) {
        if (!this.weeklyWidgetContainer) return;
        const data = await (statsService || this.statsService).getCategorySetsData(7); // Больше не передаем cachedAllWorkouts
